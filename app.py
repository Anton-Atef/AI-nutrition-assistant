...
# =========================================================
# 🔧 OCR ENGINE + PREPROCESSING (FIXED SECTION)
# =========================================================
try:
    import cv2
    HAS_CV2 = True
...
@st.cache_resource
def load_ocr_engine():
    """Try EasyOCR first (best accuracy), fall back to pytesseract, else None."""
    try:
        import easyocr
...
def preprocess_label_image(pil_img: Image.Image) -> Image.Image:
    """Upscale + denoise + binarize a photographed nutrition label to boost OCR accuracy."""
    if HAS_CV2:
...
def run_ocr_on_image(pil_img: Image.Image, ocr_engine) -> str:
    """Run OCR with the available engine and return raw text."""
    processed = preprocess_label_image(pil_img)
    if ocr_engine is None:
        return ""

    if ocr_engine["engine"] == "easyocr":
        arr = np.array(processed.convert("RGB"))
        try:
            texts = ocr_engine["reader"].readtext(arr, detail=0, paragraph=True)
        except Exception:
            try:
                texts = ocr_engine["reader"].readtext(arr, detail=0)
            except Exception:
                texts = []
        return "\n".join(texts)

    elif ocr_engine["engine"] == "tesseract":
...
def normalize_ocr_text(text: str) -> str:
    ...
    t = t.replace("\n", " ")
    t = re.sub(r"\s+", " ", t)
    t = re.sub(r"(?<=\d)o(?=\d)", "0", t)          # "1o0" -> "100"
    t = re.sub(r"(?<=\d)l(?=\s|g|mg)", "1", t)     # "10l" -> "101"
    t = t.replace(",", ".")
    return t

# --------- OCR PARSER (FIXED: normalization + safer patterns) ---------
def parse_nutrition_label_text(text: str) -> Dict:
    ...
    out = {
        "calories": find(r"calories?(?!\s*from)[^\d]{0,15}(\d+\.?\d*)"),
        "total_fat_g": find(r"total\s*fat[^\d]{0,10}(\d+\.?\d*)\s*g"),
        "saturated_fat_g": find(r"sat(?:urated)?\.?\s*fat[^\d]{0,10}(\d+\.?\d*)"),
        "trans_fat_g": find(r"trans\s*fat[^\d]{0,10}(\d+\.?\d*)"),
        "cholesterol_mg": find(r"cholesterol[^\d]{0,10}(\d+\.?\d*)\s*mg"),
        "sodium_mg": find(r"sodium[^\d]{0,10}(\d+\.?\d*)\s*mg"),
        "total_carbohydrates_g": find(r"total\s*carb(?:ohydrate)?s?[^\d]{0,10}(\d+\.?\d*)\s*g"),
        "dietary_fiber_g": find(r"(?:dietary\s*)?fib(?:er|re)[^\d]{0,10}(\d+\.?\d*)\s*g"),
        "total_sugars_g": find(r"total\s*sugars?[^\d]{0,10}(\d+\.?\d*)\s*g"),
        "added_sugars_g": find(r"added\s*sugars?[^\d]{0,10}(\d+\.?\d*)\s*g"),
    ...
    if out["total_sugars_g"] is None:
        out["total_sugars_g"] = find(r"(?<!added )sugars?[^\d]{0,10}(\d+\.?\d*)\s*g")

    sm = re.search(r"serving size[^\d]{0,20}([\d./]+\s*\w*\s*\(?\d*\s*g?\)?)", t)
    if sm:
        out["serving_size"] = sm.group(1).strip()

    spc = re.search(r"servings?\s*per\s*container[^\d]{0,10}(\d+)", t)
    if spc:
        out["servings_per_container"] = float(spc.group(1))

    return out

# =========================================================
# END FIXED OCR SECTION
# =========================================================
...
# ===== TAB 2 Label Scanner (FIXED) =====
with tabs[1]:
    st.subheader("Nutrition Facts Label Detection + OCR")

    if ocr_reader is None:
        st.warning("⚠️ No OCR engine detected (`easyocr` / `pytesseract` not installed). "
                    "Add `easyocr` or `pytesseract` + `tesseract-ocr` to requirements — "
                    "you can still enter values manually below after uploading a photo.")
    else:
        st.caption(f"OCR engine active: **{ocr_reader['engine']}**")
        ...
        if img2_file:
            ...
            if st.button("🔍 Extract with OCR", type="primary"):
                with st.spinner("Running OCR..."):
                    raw_text = run_ocr_on_image(pil2, ocr_reader) if ocr_reader else ""
                    if not raw_text.strip():
                        st.warning("OCR returned no usable text. Try a sharper, well-lit photo, or fill values manually below.")
                    parsed = parse_nutrition_label_text(raw_text)
                    st.session_state["last_label"] = parsed
                    st.session_state["last_label_raw"] = raw_text
        else:
    ...
    # ---- Persistent results area (outside button click -> fixes nested-button bug) ----
    if st.session_state.get("last_label") is not None:
        parsed = st.session_state["last_label"]
        raw_text = st.session_state.get("last_label_raw", "")

        st.markdown("---")
        st.markdown("#### 📋 Extracted Values — review & correct if needed")

        with st.expander("🔍 Debug: Raw OCR Text"):
            st.text(raw_text if raw_text.strip() else "No text captured.")

        fields = [
            "calories","total_fat_g","saturated_fat_g","trans_fat_g","cholesterol_mg","sodium_mg",
            "total_carbohydrates_g","dietary_fiber_g","total_sugars_g","added_sugars_g","protein_g"
        ]
        edit_df = pd.DataFrame({"Nutrient": fields, "Value": [parsed.get(f) for f in fields]})
        edited = st.data_editor(edit_df, use_container_width=True, num_rows="fixed", key="label_editor")

        cc1, cc2 = st.columns(2)
        with cc1:
            prod_name = st.text_input("Product name for log", value="Packaged Product (OCR)")
        with cc2:
